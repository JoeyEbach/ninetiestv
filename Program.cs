...
{
    List<Show> Ratings = shows.OrderBy(s => s.ImdbRating).ToList();
    return Ratings.Skip(1).ToList();
}
...
{
    List<Show> comedy = shows.Where(s => s.Genres.Contains("Comedy")).OrderBy(p => p.ImdbRating).ToList();
...
{
    List<Show> manyGenres = shows.Where(s => s.Genres.Count > 1).ToList();
    return manyGenres.OrderBy(s => s.StartYear).ToList();
}

// 15. Return the show with the most episodes.
static Show MostEpisodes(List<Show> shows)
{
    int highestEpisodeCount = shows.Max(s => s.EpisodeCount);
    return shows.FirstOrDefault(s => s.EpisodeCount == highestEpisodeCount);
}
...
{
    List<Show> endYearOrder = shows.OrderBy(s => s.EndYear).ToList();
...
{
    List<Show> ratings = shows.OrderByDescending(s => s.ImdbRating).ToList();
    return shows.FirstOrDefault(s => s.Genres.Contains("Drama"));
}
...
{
    List<Show> crime = shows.Where(s => s.Genres.Contains("Crime") && s.ImdbRating > 7).ToList();
    return crime.Count;
}
...
{
    List<Show> longRunning = shows.Where(s => (s.EndYear - s.StartYear) > 10 && s.ImdbRating < 8).OrderBy(s => s.Name).ToList();
    return longRunning[0];
}

// 21. Return the show with the most words in the name.
static Show WordieastName(List<Show> shows)
{
    List<Show> names = shows.OrderByDescending(x => x.Name.Count(s => s == ' ')).ToList();
    return names.First();
}
...
static string AllNamesWithCommas(List<Show> shows)
{
    ...
    return showNames.Aggregate((current, next) => current + ", " + next);
}
...
{
    List<string> showNames = shows.Select(s => s.Name).ToList();
    string lastName = showNames.Last();
    showNames.RemoveAt(showNames.Count - 1);
    string combine = showNames.Aggregate((current, next) => current + ", " + next);
